...
import '../../features/encomendas/encomenda_entrega_screen.dart';
import '../../features/encomendas/encomenda_selecao_pessoa_screen.dart';
...
// Adicionei o parâmetro condominioId na função chamadora
        ...
        Container(
          ...
          child: Column(
            children: [
              ...
              CharacterCounterField(
                controller: _codigoBarrasEncomendaController,
                enabled: _selectedUnidadesEncomenda.length <= 1,
                labelText: 'Código de Barras',
                maxLength: 100,
                decoration:
                    inputDecorationPadrao(context, hintText: 'Código de Barras')
                        .copyWith(
                  suffixIcon: Icon(
                    Symbols.barcode,
                    color: getSecondaryTextColor(context),
              ...
              CharacterCounterField(
                controller: _identificacaoInternaEncomendaController,
                enabled: _selectedUnidadesEncomenda.length <= 1,
              ...
              const SizedBox(height: 8),
              // Multi-seleção de Unidades (até 10)
              LayoutBuilder(
                builder: (context, constraints) {
                  Widget unidadeWidget = _loadingUnidadesEncomenda
                      ? const Center(child: CircularProgressIndicator())
                      : _unidadesEncomendaList.isEmpty
                          ? Container(
                              padding: const EdgeInsets.all(12),
                              decoration: BoxDecoration(
                                color: getSecondaryBackgroundColor(context),
                                borderRadius: BorderRadius.circular(8),
                                border:
                              ...
                              child: Column(
                                ...
                                children: [
                                  SizedBox(
                                    height: 32,
                                    child: ElevatedButton.icon(
                                      onPressed: _loadingUnidadesEncomenda
                                          ? null
                                          : _fetchUnidadesEncomenda,
                                      icon: _loadingUnidadesEncomenda
                                          ? const SizedBox(
                                              width: 16,
                                              height: 16,
                                              child: CircularProgressIndicator(
                          ...
                          : Column(
                              ...
                              children: [
                                ...
                                if (_selectedUnidadesEncomenda.isNotEmpty)
                                  Padding(
                                    ...
                                    child: Wrap(
                                      ...
                                      runSpacing: 4,
                                      children:
                                          _selectedUnidadesEncomenda.map((u) {
                                        final label = unidadeLabelComMorador(u);
                                        return Chip(
                                          label: Text(
                                            label,
                                            style:
                                ...
                                // Indicador de limite
                                if (_selectedUnidadesEncomenda.length >= 10)
                                  Padding(
                                    padding: const EdgeInsets.only(bottom: 6),
                                    child: Text(
                                ...
                                // Campo de busca para adicionar unidade
                                if (_selectedUnidadesEncomenda.length < 10)
                                  LayoutBuilder(
                                    ...
                                    return buildStandardAutocomplete<
                                        Map<String, dynamic>>(
                                      key: ValueKey(
                                          'multi_${_selectedUnidadesEncomenda.length}'),
                                      context: context,
                                      labelText: 'Unidade',
                                      items: _unidadesEncomendaList
                                          .where((u) =>
                                              !_selectedUnidadesEncomenda.any(
                                                  (s) =>
                                                      s['apto_id'] ==
                                                      u['apto_id']))
                                          .toList(),
                                      ...
                                      onSelected: (value) {
                                        if (value != null &&
                                            _selectedUnidadesEncomenda.length <
                                                10) {
                                          ...
                                          if (!jaAdicionada) {
                                            setState(() {
                                              _selectedUnidadesEncomenda
                                                  .add(value);
                                              if (_selectedUnidadesEncomenda
                                                      .length >
                                                  1) {
                                                _codigoBarrasEncomendaController
                                                    .clear();
                                                _identificacaoInternaEncomendaController
                                                    .clear();
                                              }
                                            });
                                          }
                  ...
                  Widget tipoWidget = _loadingTiposEncomenda
                      ? const Center(child: CircularProgressIndicator())
                      : _tiposEncomendaList.isEmpty
                          ? Container(
                              padding: const EdgeInsets.all(12),
                              decoration: BoxDecoration(
                                color: getSecondaryBackgroundColor(context),
                              ...
                              child: Column(
                                ...
                                children: [
                                  SizedBox(
                                    height: 32,
                                    child: ElevatedButton.icon(
                                      onPressed: _loadingTiposEncomenda
                                          ? null
                                          : _fetchTiposEncomenda,
                                      icon: _loadingTiposEncomenda
                                          ? const SizedBox(
                                              width: 16,
                                              height: 16,
                                              child: CircularProgressIndicator(
                          ...
                          : LayoutBuilder(builder: (context, constraints) {
                              return buildStandardAutocomplete<
                                  Map<String, dynamic>>(
                                key: ValueKey(_tipoEncomendaSelecionado),
                                context: context,
                                labelText: 'Tipo',
                                items: _tiposEncomendaList,
                                itemAsString: (item) =>
                                    (item['descricao'] ?? '').toString(),
                                selectedItem: _tipoEncomendaSelecionado,
                                onSelected: (value) {
                                  setState(() {
                                    _tipoEncomendaSelecionado = value;
                                  });
                                },
                                constraints: constraints,
                              );
                  ...
                  Widget localWidget = _loadingLocaisEntrega
                      ? const SizedBox(
                          height: 48,
                          child: Center(
                      ...
                      : LayoutBuilder(builder: (context, constraints) {
                          return buildStandardAutocomplete<
                            ...
                            context: context,
                            labelText: 'Local',
                            items: _locaisEntregaList,
                            itemAsString: (item) =>
                                (item['descricao'] ?? '').toString(),
                            selectedItem: _localEntregaSelecionado,
                            key: ValueKey(_localEntregaSelecionado),
                            onSelected: (value) {
                              setState(() {
                                _localEntregaSelecionado = value;
                              });
                            },
                            constraints: constraints,
                          );
                  ...
                  return Column(
                    children: [
                      unidadeWidget,
                      const SizedBox(height: 8),
                      // Sempre exibir Tipo e Local na mesma linha
                      Row(
                        children: [
                          Expanded(child: tipoWidget),
                          const SizedBox(width: 8),
                          Expanded(child: localWidget),
                        ],
                      ),
                      const SizedBox(height: 8),
                      // Campo Observação com botões ao lado
                      Row(
                        children: [
                          Expanded(
                            child: CharacterCounterField(
                              controller: _observacaoEncomendaController,
                              labelText: 'Observação',
                              maxLength: 255,
                              decoration: inputDecorationPadrao(
                                context,
                        ...
                        children: [
                          TransparentIconGroup([
                            IconActionData(
                              ...
                              tooltip: 'Limpar formulário',
                              color: IconColors.delete(context),
                              onPressed: _limparFormularioEncomenda,
                            ),
                            IconActionData(
                              icon: Symbols.photo_camera,
                              tooltip: 'Capturar foto da encomenda',
                              color: IconColors.camera(context),
                              onPressed: _loadingFotoEncomenda
                                  ? () {}
                                  : _tirarFotoEncomenda,
                              isLoading: _loadingFotoEncomenda,
                            ),
                            IconActionData(
                              icon: Symbols.send,
                              tooltip: 'Enviar encomenda para unidade',
                              color: Colors.green,
                              onPressed: _enviarEncomenda,
                              isLoading: _loadingEnviarEncomenda,
                            ),
                          ]),
                        ],
                      ),
                      if (_feedbackMessageRegistroEncomenda.isNotEmpty)
                        Padding(
              ...
              // Foto da encomenda: mostrar somente após captura
              if (_fotoEncomenda != null) ...[
                const SizedBox(height: 16),
                Container(
                  padding: const EdgeInsets.all(16),
                  decoration: BoxDecoration(
      ...
      child: Column(
        children: [
          ...
          Container(
            ...
            child: Column(
              children: [
                // Buscar por código de barras com botões ao lado
                // Campo de filtro por unidade (sem botões)
                CharacterCounterField(
                  controller: _filtroNomeEntregaController,
                  labelText: 'Unidade',
                  maxLength: 50,
                  decoration:
                      inputDecorationPadrao(context, hintText: 'Unidade'),
                  onChanged: (value) {
                    // Removido onChanged automático - só filtra no botá£o
                  },
                ),
                const SizedBox(height: 8),

                // Identificação Interna (filtro Entregar)
                CharacterCounterField(
                  controller: _identificacaoInternaEntregaController,
                  ...
                  onSubmitted: (_) => _fetchHistoricos(),
                ),
                const SizedBox(height: 8),
                ...
                Row(
                  children: [
                    Expanded(
                      child: CharacterCounterField(
                        controller: _filtroCodigoBarrasEntregaController,
                        labelText: 'Código de Barras',
                        maxLength: 100,
                        decoration: inputDecorationPadrao(
                          context,
                ...
                Row(
                  ...
                  children: [
                    TransparentIconGroup([
                      // Botá£o para filtrar entregues (com seleçao de período)
                      IconActionData(
                        icon: Symbols.ink_eraser,
                        ...
                        onPressed: () {
                          setState(() {
                            _filtroNomeEntregaController.clear();
                            _filtroCodigoBarrasEntregaController.clear();
                            _identificacaoInternaEntregaController.clear();
                            _filtroEntregues = false;
                            _filtroDataInicioEntrega = null;
                            _filtroDataFimEntrega = null;
                          });
                          _fetchHistoricos();
                        },
                      ),
                      IconActionData(
                        icon: Symbols.search,
                        tooltip: 'Filtrar encomendas',
                        color: IconColors.search(context),
                        onPressed: () {
                          _fetchHistoricos();
                        },
                        isLoading: _loadingHistoricoEncomendas,
                      ),
                      IconActionData(
                        icon: Symbols.assignment_turned_in,
                        tooltip: 'Filtrar Entregues (Período)',
                        isMarked: _filtroEntregues,
                        color: _filtroEntregues
                            ? const Color(0xFF2E74FF)
                            : Colors.grey.shade600,
                        onPressed: () {
                          setState(() {
                            _showFiltroEntreguesPeriodo =
                                !_showFiltroEntreguesPeriodo;
                          });
                        },
                      ),
                ...
                if (_showFiltroEntreguesPeriodo)
                  Padding(
                    ...
                    child: Container(
                      ...
                      child: StableRangeCalendar(
                        onDone: (start, end) {
                          setState(() {
                            _filtroEntregues = true;
                            _filtroDataInicioEntrega = start;
                            _filtroDataFimEntrega = end;
                            _showFiltroEntreguesPeriodo = false;
                          });
                          _fetchHistoricos();
                        },
                      ),
          ...
          const SizedBox(height: 12),
          Expanded(
            child: _loadingHistoricoEncomendas
                ? const Center(child: CircularProgressIndicator())
                : _historicoEncomendasList.isEmpty
                    ? const Center(
                        child: Column(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
                    ...
                    : ListView.separated(
                        itemCount: _historicoEncomendasList.length,
                        separatorBuilder: (_, __) => const SizedBox(height: 10),
                        itemBuilder: (context, index) {
                          final entrega = _historicoEncomendasList[index];

                          // Extrair unidade e nome da pessoa
                          final unidadeCompleta =
                              entrega['unidade']?.toString() ?? '';
                          final partesUnidade = unidadeCompleta.split('<br>');
                          final unidadeLimpa = partesUnidade.isNotEmpty
                              ? partesUnidade[0].trim()
                              : 'Unidade';
                          final nomePessoa = partesUnidade.length > 1
                              ? partesUnidade[1].trim()
                              : '';

                          // Se estiver filtrando por entregues, mostrar quem retirou e data
                          final quemRetirou =
                              entrega['quemrecebeu']?.toString() ??
                                  entrega['quemretirou_txt']?.toString() ??
                                  '';
                          final dataEntrega =
                              entrega['dataentrega']?.toString() ??
                                  entrega['dt_entrega']?.toString() ??
                                  '';
                          final dataFormatada = _formatDate(dataEntrega);

                          // Se entregue, mostrar Unidade no título e (Retirado por + Data) no subtítulo
                          // Ou conforme pedido: "unidade" nos cards e data de entrega "dt_entrega"

                          final itemKey = (entrega['aviso_id'] ??
                                  entrega['avisoentrega_id'] ??
                                  entrega['id'] ??
                                  index)
                              .toString();

                          final bool isEntregue = (entrega['statusentrega_id']
                                      ?.toString() ==
                                  '123' ||
                              entrega['statusentrega']?.toString() == '123' ||
                              _filtroEntregues);

                          final feedbackMessage =
                              _itemFeedbackMessages[itemKey];

                          final bool revealNome =
                              _revealedPii.contains('${itemKey}_nome');
                          final bool revealRetirou =
                              _revealedPii.contains('${itemKey}_retirou');

                          final titleDisplay = _filtroEntregues
                              ? 'Unidade: $unidadeLimpa'
                              : unidadeLimpa;
                          final subtitleDisplay = _filtroEntregues
                              ? 'Entregue em: $dataFormatada\nRetirado por: ${revealRetirou ? quemRetirou : _maskName(quemRetirou)}'
                              : (revealNome
                                  ? nomePessoa
                                  : _maskName(nomePessoa));

                          return _listItem(
                            leading: Symbols.inventory_2, // Conforme PRD
                            leadingColor: isEntregue
                                ? Colors.green.shade700
                                : const Color(0xFF684F8E), // Cores do PRD
                            title: titleDisplay,
                            subtitle: subtitleDisplay, // Fallback
                            uniqueKey: itemKey,
                            subtitleWidget: GestureDetector(
                              onTap: () {
                                setState(() {
                                  final key = isEntregue
                                      ? '${itemKey}_retirou'
                                      : '${itemKey}_nome';
                                  if (_revealedPii.contains(key)) {
                                    _revealedPii.remove(key);
                            ...
                            userData: entrega, // Para o avatar/foto
                            userId: (entrega['usuario_id'] ??
                                    entrega['usuario_para_id'])
                                ?.toString(),
                            footer: AnimatedSize(
                              duration: const Duration(milliseconds: 250),
                              curve: Curves.easeOutCubic,
                              alignment: Alignment.topCenter,
                              child: _cancellingItemId == itemKey
                                  ? _buildCancelInlineForm(entrega, itemKey)
                                  : (feedbackMessage != null
                                      ...
                                      : const SizedBox.shrink(
                                          key: ValueKey('empty_feedback'))),
                            ),
                            trailing: TransparentIconGroup(
                              isEntregue
                                  ? [
                                      IconActionData(
                                        ...
                                        onPressed: () =>
                                            _visualizarPdfEncomenda(entrega),
                                      ),
                                      IconActionData(
                                        ...
                                        onPressed: () =>
                                            _mostrarDetalhesEntrega(entrega),
                                      ),
                                    ]
                                  : [
                                      IconActionData(
                                      ...
                                      IconActionData(
                                        icon: Symbols.hand_package,
                                        tooltip: 'Entregar encomenda',
                                        color: const Color(
                                            0xFF28A745), // Sucesso do PRD
                                        onPressed: () =>
                                            _abrirTelaEntregaEncomenda(entrega),
                                      ),
                                      IconActionData(
                                        icon: Icons.send,
                                        tooltip: 'Reenviar entrega',
                                        color: const Color(
                                            0xFF684F8E), // Primary do PRD
                                        onPressed: () =>
                                            _reenviarEntrega(entrega),
                                      ),
                                      IconActionData(
                                        icon: Icons.close,
                                        tooltip: 'Cancelar entrega',
                                        color: const Color(
                                            0xFFDC3545), // Erro do PRD
                                        onPressed: () =>
  ...
  // Funçao para abrir tela de entrega de encomenda
  void _abrirTelaEntregaEncomenda(Map<String, dynamic> encomenda) async {
    // Carregar fotos da encomenda
    _carregarFotosEntrega(encomenda);

    // Aguardar um pouco para as fotos carregarem
    await Future.delayed(const Duration(milliseconds: 500));

    // Inicializar o notifier com a foto atual
    _fotoMoradorNotifier.value = _fotoFacialEntrega;

    _abrirPainelLateral(
      EncomendaEntregaScreen(
        encomenda: encomenda,
        fotoMorador: _fotoFacialEntrega,
        fotoEncomenda: _fotoEncomendaEntrega,
        onClose: _fecharPainelLateral,
        onSelecionarPessoa: () => _abrirTelaSelecaoPessoa(encomenda),
        fotoMoradorNotifier: _fotoMoradorNotifier,
        onConfirmarEntrega: (usarToken, token, fotoMorador, fotoEncomenda) {
          _registrarEntregaEncomenda(
            encomenda,
            usarToken: usarToken,
            token: token,
            fotoMorador: fotoMorador,
            fotoEncomenda: fotoEncomenda,
          );
        },
      ),
    );
  ...
  void _abrirTelaSelecaoPessoa(Map<String, dynamic> encomenda) {
    _abrirPainelLateral(
      EncomendaSelecaoPessoaScreen(
        onClose: _fecharPainelLateral,
        encomenda: encomenda,
        onFotoTirada: (Uint8List foto) {
          // Atualizar a foto do morador na tela de entrega via notifier
          _fotoMoradorNotifier.value = foto;
          // Também atualizar a variável local para consistência
          _fotoFacialEntrega = foto;
          // Fechar apenas a tela de seleçao (a tela de entrega permanece aberta)
          _fecharPainelLateral();
        },
      ),
    );
